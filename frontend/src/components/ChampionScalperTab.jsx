...
export default function ChampionScalperTab({ accountInfo, onSelectSymbolAndGoToChart }) {
  ...
  const handleAddSymbol = (symToAdd) => {
    ...
    if (!strategyConfig.symbols?.includes(sym)) {
      ...
      const defLot = isBtc ? 1.0 : 0.10;
      const updatedSymbols = [...(strategyConfig.symbols || []), sym];
      const updatedLots = { ...(strategyConfig.symbol_lot_sizes || {}), [sym]: defLot };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, paddingTop: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {['BTCUSDm', 'XAUUSDm', 'GBPUSDm', 'GBPJPYm', 'USDJPYm', 'EURUSDm'].map(qs => (
              <button
                key={qs}
